...
- **设置页可配置** — 设置 → 插件 → 可配置 中的「生图插件」卡片：接口地址、API 密钥、模型名、默认尺寸（官方尺寸下拉）、默认张数，以及画质/背景/审核档位 + 输出格式/风格/水印（后三个默认关闭，需各自启用开关打开）6 个可选参数。下拉为**可输入 combo**（枚举建议 + 可手写自定义值，兼容非标准网关）。参数优先级：模型显式传参 > 设置页默认 > 留空用上游默认
...
| 张数 | `1` | 一次生成几张（1–4） |
| 画质 | 留空 | `low / medium / high`，留空用上游默认 |
| 背景 | 留空 | `transparent / opaque / auto`（图生图也适用） |
| 审核档位 | 留空 | `low / medium / high`（较新，部分网关不支持） |
| 启用输出格式参数 | 关 | 默认关闭（当前网关不兼容）；开启后下方输出格式才可编辑、发送上游、开放给模型 |
| 输出格式 | 留空 | `png / jpeg / webp`，需要上方开关开启后生效 |
| 启用风格参数 | 关 | 默认关闭（当前网关不兼容）；开启后下方风格才可编辑、发送上游、开放给模型 |
| 风格 | 留空 | `vivid / natural`，需要上方开关开启后生效 |
| 启用水印参数 | 关 | 默认关闭（当前网关不兼容）；开启后下方水印才可编辑、发送上游、开放给模型 |
| 水印 | 留空 | `triw / none / auto`，需要上方开关开启后生效 |

> **参数优先级**：模型调用时显式传入的参数 > 设置页填写的值 > 留空则由上游默认。上表可选参数在设置页用**可输入下拉**（combo）选择或直接手写自定义值（兼容非标准网关）；模型也能在 `generate_image` 里透传同名参数。`output_format` / `style` / `watermark` 三个参数**默认禁用**——当前网关对它们不兼容（忽略声明格式、报 Unknown parameter、把水印当布尔解析），关闭状态下：设置页不可编辑、不给上游发送、工具 schema 不向模型开放（模型看不到也传不了）。仅当你换用了支持这些参数的上游后，才在设置页把对应开关打开。
...
| 生成图时对话里多出一张毫不相干的图片 | c7 的 markdown 渲染器开始渲染 `![...](http(s)://...)`；模型从信封得知「图片已生成」后自己写 markdown 图片引用（URL 为模型幻觉） | 升级插件到最新版（信封与系统提示已明确告知模型「图片已直接显示，请勿在回复中引用图片」） |
