 ...
 * arrays and nested objects. Paths come from `uploaded-image` blocks and from
 * `generated-image` blocks (the work-dir copy of generated bytes, which the
 * envelope exposes to the model as an edit handle); ids come from any block
 * carrying an `attachment` (platform `image` blocks included, so native
...
export function collectFromValue(value, refs) {
  ...
  if ((value.type === 'uploaded-image' || value.type === 'generated-image')
