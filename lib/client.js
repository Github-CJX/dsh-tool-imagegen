	...
	*/
	const UploadedImageBubble = (0, react.memo)(function UploadedImageBubble(props) {
		...
		const load = (0, react.useCallback)((attachment) => {
			if (typeof sessionId === "string" && sessionId !== "" && pluginIds.has(String(attachment.attachmentId))) return Promise.resolve(`${ATTACHMENT_API.path}?session=${encodeURIComponent(sessionId)}&id=${encodeURIComponent(String(attachment.attachmentId))}`);
