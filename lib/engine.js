 ...
 * field and let the upstream default apply). The switch-gated parameters
 * (output_format / style / watermark) are forced empty while their enable
 * switch is off — they must not reach the upstream even if a stale client
 * passes them; the enable switches are set by the user in the settings card
 * (default off, because the reference gateway rejects these three).
 * @param {object} cfg - the resolved config: the six option strings plus the
 *   three `output_format_enabled` / `style_enabled` / `watermark_enabled`
 *   booleans.
...
export function resolveOptionalParams(cfg, args) {
  ...
  return {
    quality: str(args.quality) || cfg.quality,
    output_format: gated('output_format', args.output_format),
    background: str(args.background) || cfg.background,
    style: gated('style', args.style),
    moderation: str(args.moderation) || cfg.moderation,
