...
export const Config = z.object({
  ...
  // Enable switches for all six optional parameters, unified behind the
  // settings card's "check the box to use this parameter" interaction. They
  // default OFF: a disabled parameter is not editable, is never forwarded to
  // the upstream (even when a stale client passes it), and is not advertised
  // in the tool schema — the model does not know it exists. Turn one ON when
  // the configured gateway accepts it; the reference relay rejects
  // output_format ("declared type does not match its bytes"), style ("Unknown
  // parameter"), and watermark (parsed as a boolean).
  quality_enabled: z.boolean().default(false),
...
export function resolveConfig(value = {}) {
  return {
    ...
    quality_enabled: typeof value.quality_enabled === 'boolean' ? value.quality_enabled : DEFAULTS.quality_enabled,
    ...
    background_enabled: typeof value.background_enabled === 'boolean' ? value.background_enabled : DEFAULTS.background_enabled,
    style_enabled: typeof value.style_enabled === 'boolean' ? value.style_enabled : DEFAULTS.style_enabled,
    moderation_enabled: typeof value.moderation_enabled === 'boolean' ? value.moderation_enabled : DEFAULTS.moderation_enabled,
...
export function apply(ctx, config) {
  ...
  function registerTool(value) {
    ...
    // All six optional parameters are switch-gated (default OFF): advertised
    // to the model only while the user checked "use this parameter" in the
    // settings card. The reference gateway rejects three of them outright
    // (output_format / style / watermark); the other three are gated too so
    // the card interaction is uniform.
    const optionals = {
      quality: {
        type: 'string',
    ...
    for (const [name, schema] of Object.entries(optionals)) {
      if (value[`${name}_enabled`]) properties[name] = schema
    }
    ...
    disposeTool = ctx.tools.register({
      ...
      async execute(args, exec) {
        ...
        // tests (see resolveOptionalParams): every optional parameter is
        // forwarded only while its switch is on — a disabled parameter is
        // neither advertised in the tool schema nor sent upstream.
  ...
  function syncToolSchema(value) {
    const signature = `${value.quality_enabled}|${value.output_format_enabled}|${value.background_enabled}|${value.style_enabled}|${value.moderation_enabled}|${value.watermark_enabled}`
